...
- **Docker (recommandé, proche de la prod) :** `docker compose up -d` (fichier `docker-compose.yml` à la racine) — build et lance MySQL, config-server, eureka-server, gateway, microservice-produits, microservice-admin, phpMyAdmin (`:8090`) et le frontend Angular (`:8081`), tous sur le réseau bridge `easycom-network` avec des `depends_on` conditionnés par healthcheck.
