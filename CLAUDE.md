...
- **Docker (recommandé, proche de la prod) :** `docker compose up -d` (fichier `docker-compose.yml` à la racine) — build et lance MySQL, config-server, eureka-server, gateway (`:8080`), microservice-produits, microservice-admin, phpMyAdmin (`:8090`) et le frontend Angular (`:8081`), tous sur le réseau bridge `easycom-network` avec des `depends_on` conditionnés par healthcheck. **HTTPS : géré par une nginx installée directement sur l'hôte (hors Docker), qui fait aussi tourner d'autres applications sur ce même serveur** — ne pas essayer de prendre les ports 80/443 depuis un conteneur (ex. Caddy), ça couperait ces autres applications. Le HTTPS pour ce projet se fait en ajoutant un site (`server_name`) supplémentaire à cette nginx hôte, qui reverse-proxy vers `localhost:8081` (frontend, qui proxy lui-même en interne `/gateway-proxy/` et `/api/` vers le gateway, voir `nginx.conf` du frontend) et en obtenant un certificat Let's Encrypt via `certbot --nginx` pour ce nom précis. Nom de domaine en attendant le nom définitif : un sous-domaine `sslip.io` (résout automatiquement vers l'IP du serveur, aucune configuration DNS requise) — à remplacer par le vrai domaine dans ce site nginx dès qu'il est disponible, puis relancer `certbot`. `environment.prod.ts` du frontend utilise `apiUrl: ''` (chemin relatif, même origine que la page) pour que ça fonctionne en HTTPS sans contenu mixte ni CORS — indépendant de la méthode de terminaison TLS choisie.
